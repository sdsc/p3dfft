...
# This script will generate a permutative batch job script on Comet.
#
...
batchf = open('jobs/comet_permutative.sh', 'w')

# Write SBATCH header commands.
batchf.write('#!/bin/bash\n')
batchf.write('#SBATCH --job-name="p3dfft"\n')
batchf.write('#SBATCH --output="out/out.%j"\n')
batchf.write('#SBATCH --partition=compute\n')
batchf.write('#SBATCH --nodes=1\n')
batchf.write('#SBATCH --ntasks-per-node=16\n')
