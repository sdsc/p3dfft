...
!$OMP END PARALLEL 
	...
	allocate(starty_b_c2_same(0:num_thr-1),starty_f_c2_same(0:num_thr-1),starty_b_c2_dif(0:num_thr-1),starty_f_c2_dif(0:num_thr-1))
      ...
      do tid=0,l-1
         ...
         starty_frc(tid+1) = startx_frc(tid) + (m+1)*nxhp 		
         starty_bcr(tid+1) = startx_frc(tid) + (m+1)*nx_fft 		
         startx_bcr(tid+1) = startx_frc(tid) + (m+1)*nxhp 		
      enddo		         
      do tid=l,num_thr-1
         startx_frc(tid+1) = startx_frc(tid) + m*nx_fft
         starty_frc(tid+1) = startx_frc(tid) + m*nxhp
         starty_bcr(tid+1) = startx_frc(tid) + m*nx_fft
         startx_bcr(tid+1) = startx_frc(tid) + m*nxhp
      enddo		         
	...
	print *,taskid, ': doing plan_f_c1'
     ...
     if(iisize*kjsize .gt. 0) then 
      ...
      do tid=0,l-1
         ...
         starty_f_c1(tid+1) = startx_f_c1(tid) + (m+1)*ny_fft 		
         starty_b_c1(tid+1) = startx_b_c1(tid) + (m+1)*ny_fft 		
         startx_b_c1(tid+1) = startx_b_c1(tid) + (m+1)*ny_fft 		
      enddo		         
      do tid=l,num_thr-1
         startx_f_c1(tid+1) = startx_f_c1(tid) + m*ny_fft
         starty_f_c1(tid+1) = startx_f_c1(tid) + m*ny_fft
         starty_b_c1(tid+1) = startx_b_c1(tid) + m*ny_fft
...
!$OMP END PARALLEL
     ...
     if(iisize .gt. 0) then
      ...
      do tid=0,l-1
         ...
         starty_f_c1(tid+1) = startx_f_c1(tid) + (m+1)
         starty_b_c1(tid+1) = startx_b_c1(tid) + (m+1)
         startx_b_c1(tid+1) = startx_b_c1(tid) + (m+1)
      enddo		         
      do tid=l,num_thr-1
         startx_f_c1(tid+1) = startx_f_c1(tid) + m
         starty_f_c1(tid+1) = startx_f_c1(tid) + m
         starty_b_c1(tid+1) = startx_b_c1(tid) + m
...
!$OMP END PARALLEL
      ...
      do tid=0,l-1
         ...
         starty_f_c2_same(tid+1) = startx_f_c2_same(tid) + (m+1)
         starty_b_c2_same(tid+1) = startx_b_c2_same(tid) + (m+1)
	 ...
	 starty_strans_same(tid+1) = starty_strans_same(tid) + (m+1)*2
      ...
      do tid=l,num_thr-1
         ...
         starty_f_c2_same(tid+1) = startx_f_c2(tid)_same + m
         starty_b_c2_same(tid+1) = startx_b_c2(tid)_same + m
