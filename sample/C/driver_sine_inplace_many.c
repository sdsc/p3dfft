...
int main(int argc,char **argv)
{
    ...
    if(proc_id == 0) {
        ...
        printf("P3DFFT test, 3D wave input, in-place transform, multivariable version\n");
#ifndef SINGLE_PREC
...
#ifndef SINGLE_PREC
    A = (double *) malloc(sizeof(double) * nm * nv);
#else
    A = (float *) malloc(sizeof(float) * nm * nv);
#endif
    ...
    for(m=0;m < n;m++) {
        ...
        for (j=0; j < nv; j++) {
            if(proc_id == 0)
                printf("Result of forward transform, var. %d\n", nv);

            print_all(A,Ntot,j,proc_id,Nglob);
            /* normalize */
            mult_array(A,Ntot,j,factor);
        }
